...
func TextHelper(c *gin.Context) (openaiErr *dto.OpenAIErrorWithStatusCode) {
	...
	if resp != nil {
		...
		if httpResp.StatusCode != http.StatusOK {
			...
			common.SysLog(fmt.Sprintf("重置状态码1: %d", httpResp.StatusCode))
	...
	if openaiErr != nil {
		// reset status code 重置状态码
		common.SysLog(fmt.Sprintf("重置状态码2: %d", httpResp.StatusCode))
		service.ResetStatusCode(openaiErr, statusCodeMappingStr)
		return openaiErr
	}
	...
	if strings.HasPrefix(relayInfo.UpstreamModelName, "gpt-4o-audio") {
		...
		go postConsumeQuota(c, relayInfo, textRequest.Model, usage.(*dto.Usage), ratio, preConsumedQuota, userQuota, modelRatio, groupRatio, modelPrice, getModelPriceSuccess, "")
	}
	return nil
}
