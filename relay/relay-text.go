...
func TextHelper(c *gin.Context) (openaiErr *dto.OpenAIErrorWithStatusCode) {
	...
	if strings.HasPrefix(relayInfo.OriginModelName, "gpt-4o-audio") {
		...
		go postConsumeQuota(c, relayInfo, usage.(*dto.Usage), preConsumedQuota, userQuota, priceData, "")
	}
	return nil
}
...
func preConsumeQuota(c *gin.Context, preConsumedQuota int, relayInfo *relaycommon.RelayInfo) (int, int, *dto.OpenAIErrorWithStatusCode) {
	...
	if userQuota <= 0 {
		errorMsg := fmt.Sprintf("user(%d) quota is not enough", relayInfo.UserId)
		return 0, 0, service.OpenAIErrorWrapperLocal(errors.New(errorMsg), "insufficient_user_quota", http.StatusForbidden)
	}
