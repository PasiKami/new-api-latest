...
# - do not add inbound firewall blocks unless APPLY_INBOUND_FIREWALL=1
...
APPLY_INBOUND_FIREWALL="${APPLY_INBOUND_FIREWALL:-0}"
...
install_firewall_blocks() {
  ...
  mkdir -p /etc/default
  {
    ...
    printf "APPLY_INBOUND_FIREWALL='%s'\n" "$APPLY_INBOUND_FIREWALL"
...
if command -v iptables >/dev/null 2>&1; then
  remove_protected_inbound_blocks
  if [ "$APPLY_INBOUND_FIREWALL" = "1" ]; then
